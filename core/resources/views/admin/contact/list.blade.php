                    ...
                    <form action="">
                        <div class="align-items-center justify-content-end d-flex flex-wrap gap-2">
                            <div class="flex-fill">
                                ...
                                <input type="name" autocomplete="off" name="name" value="{{ request()->name ?? null }}"
                                    placeholder="@lang('Search with name')" value="" class="form-control">
                            </div>
                            @if ($emailShow)
                                <div class="flex-fill">
                                    <label>@lang('Email')</label>
                                    <input type="text" autocomplete="off" name="email"
                                        value="{{ request()->email ?? null }}" placeholder="@lang('Search with email')"
                            ...
                            @if ($mobileShow)
                                <div class="flex-fill">
                                    ...
                                    <input type="text" autocomplete="off" name="mobile"
                                        value="{{ request()->mobile ?? null }}" placeholder="@lang('Search with mobile')"
                                        value="" class="form-control">
                                </div>
                            @endif
                        ...
                        <table class="table--light style--two table">
                            ...
                            <tbody>
                                @forelse($contacts as $contact)
                                    <tr>
                                        <td>{{ __($contacts->firstItem() + $loop->index) }}
                                        </td>
                        ...
                        @if ($mobileShow)
                            <div class="form-group">
                                ...
                                <input type="tel" class="form-control form-control-lg" name="mobile"
                                    value="{{ old('mobile') }}" placeholder="@lang('Mobile')">
                            </div>
                        @endif
                    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form method="post" action="{{ route('admin.contact.export') }}" id="importForm"
                                ...
                                @foreach ($columns as $column)
                                    <div class="{{ $loop->last && $loop->odd ? 'col-lg-12' : 'col-lg-6' }} mb-3">
                                        ...
                                        <input type="checkbox" data-width="100%" data-size="large"
                                            ...
                                            data-height="50" data-on="@lang('Yes')" data-off="@lang('No')"
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control form-control-lg" name="export_item">
                                ...
                                @if ($contacts->total() > 100)
                                    <option value="{{ $contacts->total() }}">{{ __($contacts->total()) }}
                                        @lang('Contacts')
                                    </option>
                                @endif
                            </select>
                    ...
                    <div class="modal-footer">
                        <button type="Submit"
                            class="btn btn--primary w-100 h-45 contactExport">@lang('Export')</button>
                    </div>
                </form>
            </div>
        </div>
...
@push('breadcrumb-plugins')
    <button type="button" class="btn btn-outline--primary btn-sm px-3 addBtn ">
        <i class="las la-plus"></i> @lang('New')
    </button>
    <button type="button" class="btn btn-outline--info btn-sm px-3 importBtn  ">
        <i class="las la-cloud-upload-alt"></i> @lang('Import')
    </button>
    <button type="button" class="btn btn-outline--warning btn-sm px-3 exportBtn">
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            notify('error', response.errors && response.errors.length > 0 ? response
                                .errors : response.message || "@lang('Something went the wrong')");
                        }
                    },
